...
export default function EditEmployeeModal({ visible, onClose, employee }) {
  ...
  const { mutate, isPending, error, isError } = useMutation({
    mutationFn: (data) => updateEmployeeInfo(employee._id, data),
    meta: { successMessage: 'Employee updated successfully' },
    onSuccess: () => {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="bg-background flex-1">
        <SafeAreaView style={{ flex: 1 }} edges={['top']}>
          <View className="border-border flex-row items-center justify-between border-b px-4 py-4">
            <Text className="text-foreground text-lg font-bold">Edit Employee</Text>
